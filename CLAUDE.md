...
With metronome, master volume, EQ, BPM detection, and speed ramping closed, the only remaining v0.3 thread is **export looped section as audio**. Detour clause for the DSP family still stands: if WSOLA + PV miss on a particular kind of material, Phase 5 (FFI to Rubber Band or signalsmith-stretch) is the escape hatch — the trait shape stays the same.
