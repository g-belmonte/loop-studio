...
`README.md` tracks the roadmap as a checkbox list (v0.1, v0.2, and v0.3 are closed; new work lands in v0.4). Tick the box in the same change that implements the item.

## Next planned work

v0.1, v0.2, and v0.3 are closed. v0.3 features — see the matching decision entries in ARCHITECTURE.md for the full design rationale:
...
- **BPM detection** — `analysis::bpm::detect_bpm` runs offline on a worker thread spawned by `App::spawn_bpm_detect` (loop region if set, else whole track). Spectral-flux onset envelope (Hann 1024 / hop 512 via `realfft`), mean-subtract + half-wave rectify, biased autocorrelation with octave-resolve. Result lands via crossbeam into `App::bpm_status`, reset to `Idle` on every LoadTrack, not persisted. UI in `ui::metronome` — Detect button, status, `Use` copies to `settings.bpm`; surfaced as `MetronomeAction::DetectBpm` and acted on after the central-panel closure to avoid a `&mut self` re-borrow.
- **Speed ramping** — `engine::speed_ramp::SpeedRampSettings { enabled, target_speed, step_unit (Percent | Bpm), step_amount, passes_per_step }` is pushed via `Command::SetSpeedRamp`. Worker counts loop wraps (in the stitch branch of `produce()`) and calls `advance_speed_ramp` after `push_slice` to bump speed toward target by one step (clamped at target); BPM-unit step resolves via `metronome.bpm()`. The UI slider follows because it binds to `speed_bits` every frame. `ui::transport::show` returns `TransportResult { speed_user_changed }`; on a true value `App` clears `speed_ramp.enabled` and re-sends `SetSpeedRamp` so a manual slider drag wins over the ramp. Session schema v6 (`speed_ramp`).
- **Multiple saved loops** — `App::loops: Vec<NamedLoop>` carries up to 9 slots (UI source of truth, parallel to `markers`); `App::active_loop: Option<usize>` indexes the slot driving `loop_region`. Engine state is unchanged — switching just re-sends `Command::SetLoop`. `Shift+Num1..Num9` in `ui::shortcuts` activates slot N (empty = no-op). Any region edit (`[`/`]`, waveform drag) clears `active_loop` and leaves saved slots untouched — the new region is a fresh unsaved one; users add a slot via the "Save current loop" button in `ui::loops`. `Esc` and `Clear loop` deactivate (clear `loop_region` + `active_loop`) but leave the saved list intact. Session schema v7 (`loops`, `active_loop`).
- **Export loop as WAV** — `src/render/mod.rs` renders the active loop offline on a `std::thread` spawned by `App::spawn_render`, mirroring the BPM-detect lifecycle. Builds a fresh DSP via `engine::make_dsp` (lifted from `worker.rs` so both paths share construction) plus a fresh `Eq` and `Metronome`; engine playback and cpal callback are untouched. Signal chain: DSP → EQ → optional metronome (master volume excluded by design — monitoring control, not artistic). Two passes per render — a priming pass to warm DSP state is discarded, the capture pass is written to disk; output length is `loop_length / speed` source frames per pass. Writes to `<path>.wav.part` and `fs::rename`s on `finalize()`, so a killed process never leaves a half-valid `.wav` at the chosen path. Shared `Arc<AtomicBool>` cancel flag is polled per chunk; the in-app Cancel button and `App::on_exit` both set it, and the worker removes the `.part` before returning. `on_exit` joins the worker with a 250 ms deadline so the cleanup gets a chance to run. WAV via `hound` — 16-bit PCM (truncating, no dither) or 32-bit float. Not persisted in sessions (transient action); speed-ramp settings are snapshotted at request time and don't advance during the render.

Audible verification still owed across v0.3 — enable each feature mid-track during release-build playback and confirm: metronome tracks the speed slider, beat 1 lands at `loop.start` on every wrap, tap-tempo locks in within 3–4 taps; master fader drags don't zipper, the metronome scales with it; EQ low/high shelf and mid peak each affect the expected region, each Solo button isolates that band's region, gain-slider drags are clean, enable/solo toggles produce only a brief artefact; BPM detect on a clean rock/pop track lands within ±1 of the true tempo, `Use` copies the value, and re-running after changing the loop region returns a number consistent with the passage; speed ramp on a 2-bar loop bumps the speed slider by the configured step every N wraps and stops at target, dragging the slider mid-ramp turns the ramp off, BPM-unit step at 120 BPM source yields the expected `step / 120` per-step delta; multiple loops switch with no audible gap on `Shift+N`, edits to an active slot stick when reactivated later, an autosession round-trip preserves all slots and the active index; a WAV export of an unmodified loop round-trips audibly identical to a single playback iteration of that loop, the result loops seamlessly in an external player, EQ + pitch + speed changes are baked in, the metronome checkbox actually mixes a click into the file, closing the app mid-export leaves a `.part` (never a half-valid `.wav`), and Cancel deletes the `.part` cleanly.
...
Detour clause for the DSP family still stands: if WSOLA + PV miss on a particular kind of material, Phase 5 (FFI to Rubber Band or signalsmith-stretch) is the escape hatch — the trait shape stays the same.
