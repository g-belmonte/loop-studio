...
With metronome, master volume, EQ, and BPM detection closed, ask the user which remaining v0.3 thread is next (speed ramping / export looped section). Detour clause for the DSP family still stands: if WSOLA + PV miss on a particular kind of material, Phase 5 (FFI to Rubber Band or signalsmith-stretch) is the escape hatch — the trait shape stays the same.
