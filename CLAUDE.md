...
v0.1 and v0.2 are closed; v0.3 is in flight. Done so far in v0.3:

- **Click track / metronome** — `engine::metronome::Metronome` owns a click voice + two pre-rendered exponential-decay sine bursts (1 kHz / 1.5 kHz accent); beats fire at `anchor + k * (sr * 60 / bpm)` source frames with `anchor = loop.start` (or 0 when no loop is set, updated by the worker on `LoadTrack`/`SetLoop`); mixed post-DSP into the engine's output scratch via `mix_segment`, split proportionally for stitched loop-wrap chunks so the downbeat lands at `loop.start` on every wrap. UI in `ui::metronome` (toggle / BPM / Tap button / accent / beats-per-measure / volume dB); `ui::metronome::TapTempo` averages the last 4 taps within a 2 s window. `T` key taps from `ui::shortcuts`. Session schema bumped to v4 (`metronome: MetronomeSettings`). Audible verification still owed: enable mid-track and confirm clicks track the speed slider; set a loop and confirm beat 1 lands at `loop.start` on every pass; tap a familiar tempo and confirm the click locks in within 3–4 taps.
- **Master volume** — `App::master_volume_db` drives a `ui::transport` slider (-60..=+6 dB, "0 dB" reset). `Command::SetMasterVolume(db)` converts to linear gain and parks it as the worker's `master_target_gain`; `apply_master_gain` runs after `metronome.mix_segment` and ramps `current → target` linearly across each chunk so slider drags don't zipper. Not persisted (resets to 0 dB on launch). Audible verification still owed: drag the slider fast during playback and confirm no zipper/clicks; confirm metronome scales with the master (post-metronome stage).

Older "done so far" notes from v0.2 — kept here because the implementation details still inform new work in the same areas: WSOLA quality pass (AMDF similarity over a sum-of-channels mono mix, unity-gain OLA, ramped stretch transitions); phase vocoder + `DspKind` selector (`dsp/phase_vocoder.rs` cascaded with rubato, same composite shape as `WsolaPitchShift`; selector lives in `ui::transport`); PV refinements (transient-detect-and-passthrough on a sum-of-channels mono mix and Laroche–Dolson phase locking, both confined to `phase_vocoder.rs`'s restructured `step()`); waveform zoom + scroll (`WaveformView { start, len }` on `App`, mapped to pixels by the widget; Ctrl+wheel / pinch / `+`–`-` to zoom, Shift+wheel / right-drag to pan, follow-playhead toggle pages forward when the playhead leaves the view; not persisted in sessions); per-track session auto-save (`src/session/auto.rs`, FNV-1a hash of canonical path under `$XDG_DATA_HOME/loop-studio/autosessions/`; `App::autosave_tick` debounced at `AUTOSAVE_INTERVAL = 2 s`; outgoing flush in `drain_decode_results` + `on_exit`; `open_dialog` pre-loads `PendingRestore` from `autosession::load_for`).

With metronome and master volume closed, ask the user which v0.3 thread is next (speed ramping / EQ band isolation / export looped section). Detour clause for the DSP family still stands: if WSOLA + PV miss on a particular kind of material, Phase 5 (FFI to Rubber Band or signalsmith-stretch) is the escape hatch — the trait shape stays the same.
