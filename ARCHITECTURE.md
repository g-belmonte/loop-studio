...
│
├── engine/
│   ├── mod.rs           # public Engine handle, Command/state re-exports, make_dsp (shared with offline render)
│   ├── command.rs       # Command enum sent GUI -> engine
│   ├── metronome.rs     # source-frame-anchored click scheduler + mixer
│   ├── speed_ramp.rs    # speed-ramp settings + per-step delta resolver
│   ├── state.rs         # SharedState: atomics read by GUI
│   └── worker.rs        # the engine thread loop
│
├── render/
│   └── mod.rs           # offline loop export: WAV writer + DSP/EQ/metronome on a worker thread
│
├── audio/
│   ├── mod.rs
...
- **Export loop as WAV** (decided v0.3). `src/render/mod.rs` renders the active loop offline on a `std::thread` spawned by `App::spawn_render`. Worker holds an `Arc<Track>` (read-only share with the engine), builds a *fresh* `Box<dyn TimePitchProcessor>` via `engine::make_dsp` (lifted from `engine::worker` so both the live engine and the renderer use one construction path) plus a fresh `Eq` and `Metronome` — engine state and the cpal callback are never touched, so playback can continue during a render. Signal chain mirrors the engine for everything *artistic*: DSP → EQ → optional metronome. Master volume is excluded by design (monitoring control, not signal) and the speed-ramp is snapshotted at request time (no advance during the render — a "render-with-ramp" mode is a later concern). Two passes per render: a priming pass walks the loop once and discards its output to settle WSOLA stretch-ramp and PV phase-trajectory state; the capture pass writes to disk. Loops shorter than one DSP `input_frames_per_chunk()` error out — same limitation as live playback, surfaced to the user via the `Failed` status line.
  Robustness against process kill: the writer targets `<chosen-path>.part` in the same parent directory and `fs::rename`s to the final path only after `hound::WavWriter::finalize()` succeeds. If the app is closed mid-render, `App::on_exit` sets a shared `Arc<AtomicBool>` cancel flag and joins the worker with a 250 ms deadline so the worker can drop the writer and `remove_file` the `.part` before the OS terminates the thread. The in-app Cancel button uses the same flag (same path → same cleanup → `Cancelled` event surfaced in the UI). Net effect: a half-valid `.wav` never appears at the target path; the worst residue is a visibly-incomplete `.part` file the user can delete.
  Output format: WAV via `hound`. 16-bit PCM is the default (truncating conversion with `clamp(-1.0, 1.0)`; no dither — known limitation, fine for practice exports) and 32-bit float is the bit-perfect alternative for downstream re-processing. Format + metronome-bake choice live in a `File → Export Loop as WAV…` submenu (sticky across exports within one session); the menu item is disabled when no loop is active or a render is in flight (one job at a time — the simpler concurrency story, matches `spawn_bpm_detect`). A new render gets a monotonically increasing `job_id` so the `drain_render_results` loop can discard stale events. Status is **not** persisted in sessions (transient action — a fresh app launch starts `Idle`).
  Considered and rejected: realtime capture by tapping the engine's output (ties export speed to wall clock, risks underruns, would need a second ring buffer and a "record arm" mode for a one-shot user action); rendering through the live engine's existing DSP instance (would either lock the user out of playback for the render's duration or require fork/restore of internal DSP state, both worse than just building a fresh DSP for ~zero cost); k-chunk priming instead of full-loop priming (faster, but doesn't establish PV phase state matching mid-loop playback — the full-pass approach guarantees the captured loop boundary sounds like steady-state playback); baking in master volume (it's a monitoring control, not part of the signal — putting it in the export would surprise users who pull the slider down for late-night practice and then wonder why their export is quiet); FLAC/MP3 (defer until WAV proves insufficient — WAV is universal, MP3 needs a non-trivial encoder dep); blocking app exit until export finishes (intrusive and egui's exit hooks don't cleanly support cancellation modals); a separate OS subprocess for the render (heavy, and the cancel-flag + temp-file design already solves the kill-during-render concern).
